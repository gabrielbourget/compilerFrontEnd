...
Buffer* b_create(short init_capacity, char inc_factor, char o_mode) {
	...
	/* Check that init_capacity and inc_factor provided by user meets expectations */
	if (init_capacity < 0 || inc_factor < 0) return NULL;

	/* Account for fixed mode and initial capacity <= 0 */
	if ((init_capacity <= 0) && (o_mode = 'f')) return NULL;

	/* Mode will have to be fixed if inc_factor is 0 */
	if (inc_factor == 0) {
		...
		bDescriptor->capacity = init_capacity;
	...
	case 'a':
		...
		if ((unsigned char)inc_factor <= 255) {
			/* Allocate memory for one BufferDescriptor structure */
			if ((bDescriptor = (Buffer*)calloc(1, sizeof(Buffer))) == NULL){
				return NULL;
			}
...
pBuffer b_addc(pBuffer const pBD, char symbol) {
	...
	/* If there isn't calculate how much more you need and resize it, depending on operational mode */
		...
		*****************************************/
			...
			if ((temp = (char*)realloc(pBD->cb_head, newCapacity)) == NULL){
				return NULL;
			}
		...
		***********************************************/
			...
			/* Calculate new capacity */
			newCapacity = pBD->capacity + (short)(((SHRT_MAX - pBD->capacity)*(unsigned char)pBD->inc_factor) / 100);

			/* Record current memory location of the buffer descriptor for later comparison after realloction */
			memLocationCheck = pBD->cb_head;

			/* Handles cases where the buffer can be increased in size to the new capacity just calculated */
			if (((newCapacity > 0) && (newCapacity < SHRT_MAX)) && newCapacity != pBD->capacity) {
				/* Reallocate the buffer according to the new capacity */
				...
				/* Assign cb_head to new address */
				pBD->cb_head = temp;
			}

			/* Handles case where buffer cannot be increased to the calculated capacity, increase to max size. */
			else {
				/* Reallocate the buffer according to the new capacity */
				if ((temp = (char*)realloc(pBD->cb_head, newCapacity*sizeof(char))) == NULL) {
					return NULL;
			...
			/* Add new character in the buffer, then increment addc_offset*/
			pBD->cb_head[pBD->addc_offset++] = symbol;
...
int b_isfull(Buffer* const pBD) {
	...
	/* Check to see if pointer to BufferDescriptor is null first. */
	if (pBD == NULL) return R_FAIL1;
	if (pBD->addc_offset == pBD->capacity) return 1;
	return 0;
}
...
size_t b_incfactor(Buffer* const pBD) {
	...
	/* Check to see if pointer to BufferDescriptor is null first. */
	if (pBD == NULL) return TWOFIFTYSIX;
