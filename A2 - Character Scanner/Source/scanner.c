...
int char_class(char c){
	...
	if (sym == '0')	return 1;
	if (sym >= 49 && sym <= 55) return 2;
	if (sym == 56 || sym == 57) return 3;
	if (sym == '.') return 4;
	if (sym == '%') return 5;
	return 6;
}
...
Token aa_func05(char lexeme[]){
	...
	/* Not valid integer */
		...
		if (lexlen > ERR_LEN){
			...
			strncpy(tken.attribute.err_lex, lexeme, ERR_LEN * sizeof(char));
			/* Add character to make it a c type string */
			tken.attribute.err_lex[ERR_LEN] = '\0';
		}
		else {
			/* Copy lexeme into error lexeme */
			strncpy(tken.attribute.err_lex, lexeme, lexlen * sizeof(char));
			/* Add character to make it a c type string */
